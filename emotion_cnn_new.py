...
#         self.loss, self.accuracy = self.loss_accuracy(self.output_layer(self.cnn_layer()), self.y_target)
    ...
    def cnn_activation(self, input_, weight_, bias_):
        conv = tf.nn.conv2d(input_, weight, self.strides, self.padding)
    ...
    def cnn_layer(self):
        with tf.name_scope("conv1"):
            ...
            conv1_1 = cnn_activation(self.x_input, conv1_1_weight, conv1_1_bias)
            conv1_2 = cnn_activation(conv1_1, conv1_2_weight, conv1_2_bias)
            pool1 = tf.nn.max_pool(conv2_relu, self.maxpool_ksize, self.maxpool_strides, self.padding, name = 'pool_layer1')
...
#             conv1_2 = tf.nn.conv2d(conv1_1, conv1_2_weight, conv1_1_weight, conv1_1_bias)
...
#             conv2_3_weight = self.weight_variable('conv2_3_weight', self.filter[3])
#             conv2_3_bias = self.bias_variable('conv2_3_bias', self.bias[1])
            conv2_1 = cnn_activation(dropout1, conv2_1_weight, conv2_1_bias)
            conv2_2 = cnn_activation(conv2_1, conv2_2_weight, conv2_2_bias)
#             conv2_3 = cnn_activation(, conv2_3_weight, conv2_3_bias)
...
#             norm2 = tf.nn.lrn(pool2, self.depth_radius, self.norm_bias, self.alpha, self.beta, 'norm2')
        with tf.name_scope("conv3"):
            ...
            conv3_1 = cnn_activation(dropout2, conv3_1_weight, conv3_1_bias)
            conv3_2 = cnn_activation(conv3_1, conv3_2_weight, conv3_2_bias)
            conv3_3 = cnn_activation(conv3_2, conv3_3_weight, conv3_3_bias)
        ...
        with tf.name_scope("conv4"):
            ...
            conv4_1 = cnn_activation(dropout3, conv4_1_weight, conv4_1_bias)
            conv4_2 = cnn_activation(conv4_1, conv4_2_weight, conv4_2_bias)
            conv4_3 = cnn_activation(conv4_2, conv4_3_weight, conv4_3_bias)
    ...
    def output_layer(self, fc_input):
        with tf.name_scope("output_layer"):
            with tf.name_scope("fully_connected_layer1"):
                fc_input = tf.reshape(fc_input, [-1, self.hidden_dim])
...
#             E1 = -label_*np.log(logits_) - (1 - label_)*np.log(1 - logits_)
            loss_ = -tf.reduce_sum(label_ * tf.log(logits_))
#         return loss_
#             loss_ = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits = logits_, labels = label_), name = 'cross_entropy_loss')
        with tf.name_scope("evaluate"):
            equal = tf.cast(tf.equal(tf.argmax(softmax, axis= 1), tf.argmax(label_, axis= 1)), tf.float32)
