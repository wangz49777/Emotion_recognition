...
class Emotion_cnn():
    def __init__(self):
        ...
        with tf.name_scope("y_target"):
            self.y_target = tf.placeholder(dtype=tf.int32, shape=[None, None], name='y_target') #标签
...
#             labels_ = tf.nn.softmax(tf.cast(labels_, tf.float32))
            labels_ = tf.argmax(tf.nn.softmax(tf.cast(labels_, tf.float32)), axis = 1, name = 'label')
