...
flags.DEFINE_integer("val_batch_size", 2000, "The size of a val_mini-batch.")
# flags.DEFINE_integer("batch_size", 256, "The size of a mini-batch.")
# flags.DEFINE_integer("batch_size", 256, "The size of a mini-batch.")
# flags.DEFINE_integer("batch_size", 256, "The size of a mini-batch.")
...
emo = ['neutral', 'happiness', 'surprise', 'sadness', 'anger', 'disgust', 'fear', 'contempt', 'unknown']
channel = 1  # 图像通道数
...
# for img, lab in zip(x_batch, y_label):
...
def main(argv):
    data_set_train = get_dataset(train_path) #读取训练数据
    data_set_train = data_set_train.shuffle(shuffle_pool_size).batch(FLAGS.batch_size).repeat() #打乱，分批
    data_set_train_iter = data_set_train.make_one_shot_iterator()

    data_set_val = get_dataset(val_path)
    data_set_val = data_set_val.shuffle(shuffle_pool_size).batch(FLAGS.val_batch_size).repeat()
    ...
    with tf.Session(config = config) as sess:
        ...
        for i in range(1, generations + 1):
            ...
            if i % 200 == 0:
                ...
                val_y_label = np.argmax(val_y_batch, axis= 1)
                val_feed_dict = {x_input: val_x_batch, y_target: val_y_label, dropout: 1.0}
